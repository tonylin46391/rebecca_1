...
def go_to_next_question():
    ...
    st.session_state.show_next = False # 隱藏按鈕
    st.rerun() # 觸發跳轉
...
def submit_answer():
    ...
    if user_input == current_word:
        ...
        # 🔊 播放答對音效 (呼叫 play_sound 函式，它會將音效內容存入 session state)
        play_correct_sound()
        ...
        # 🔊 播放答錯音效 (呼叫 play_sound 函式，它會將音效內容存入 session state)
        play_wrong_sound()
    ...
    # normal 模式 → 下一題 (只移動 index，不立即跳轉)
    if st.session_state.mode == "normal":
        st.session_state.index += 1

    # reset 播放狀態，並設定顯示「繼續下一題」按鈕
    st.session_state.played = False
    st.session_state.last_word = None
    st.session_state.show_next = True # 設定旗標，讓「繼續下一題」按鈕出現
    # 這裡不再有 st.rerun()，腳本會繼續執行並停在結果頁面

# 根據狀態顯示輸入表單或「繼續下一題」按鈕
if st.session_state.show_next:
    # 這裡的標題修改為更合適的提示
    st.button("👉 繼續下一題", on_click=go_to_next_question)
